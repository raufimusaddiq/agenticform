...
- [ ] Add an internal `AgentRuntime` SPI for start, dispatch, resume, and interrupt.
...
- [ ] Keep local runtime session IDs behind `RuntimeSession`; persistence exposes neutral `runtimeSessionId`.

## Codex adapter

- [ ] Add `CodexAgentRuntime` adapter over the existing App Server gateway.
- [ ] Route local agent spawn, interrupt, and task dispatch through `AgentRuntime`.
- [ ] Route local queue reconciliation and lifecycle interruption through `AgentRuntime`.
- [ ] Route local message, operation-event, and incident delivery through `AgentRuntime`.
