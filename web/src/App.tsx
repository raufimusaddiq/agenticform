...
function Agents({ agents, tasks, projectById, runOutput, onControlMode, onQueueMode, onIntervene, onRestart }: {
  ...
  return <section className="panel"><div className="section-header"><div><h2>Agents</h2></div></div>
    ...
    {selected && <section className="data-list"><h3>Live run · {selected.name}</h3><div className="agent-run-stream">{(runOutput[selected.id] ?? []).length === 0 ? <p className="muted">Waiting for agent output.</p> : runOutput[selected.id].slice(-200).map((entry, index) => <p key={index}>{entry.text}</p>)}</div></section>}
