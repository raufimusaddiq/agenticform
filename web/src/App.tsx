...
function Tasks({ tasks, projectById, agentById, onDispatch, onCreate }: { tasks: Task[]; projectById: Map<string, Project>; agentById: Map<string, Agent>; onDispatch: (id: string) => void; onCreate: () => void }) {
  const queueGroups = [
    ...
    ['Blocked', tasks.filter((task) => ['BLOCKED', 'WAITING_APPROVAL', 'PAUSED'].includes(task.status)).length],
    ['Done', tasks.filter((task) => ['COMPLETED', 'FAILED', 'CANCELLED'].includes(task.status)).length]
  ] as const;
